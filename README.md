...
```json
{
  "agents": {
    "defaults": {
      "model": "copilot/gpt-5-mini",    // 格式：provider/model-name
      "maxTokens": 8192,
      "temperature": 0.1,
      "memoryWindow": 100
    }
  ...
  "providers": {
    "custom": {
      "apiKey": "",                       // 你的 API 密钥（本地代理可选）
      "apiBase": "http://localhost:4141/v1" // 你的 OpenAI 兼容端点
    }
  }
}
```

**Model 格式**：`provider/model-name`（例如：`copilot/gpt-5-mini`、`openai/gpt-4`）
...
Feishu 凭证也可以通过环境变量 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`、`FEISHU_ENCRYPT_KEY` 提供。

如果未提供 Feishu 凭证，gateway 将仅启动 CLI 通道。
